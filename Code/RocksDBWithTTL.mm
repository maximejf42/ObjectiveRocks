...
//  ObjectiveRocks
//
//  Created by Joshua Stein on 5/10/18.
//  Copyright (c) 2018 Digita Security. All rights reserved.
//
...
@property (nonatomic, assign) rocksdb::DBWithTTL *db;
...
+ (instancetype)databaseAtPath:(NSString *)path
                       withTTL:(int32_t)ttl
                  andDBOptions:(void (^)(RocksDBOptions *))optionsBlock
{
	RocksDBWithTTL *rocks = [[RocksDBWithTTL alloc] initWithPath:path];

	if (optionsBlock) {
		optionsBlock(rocks.options);
	}

    if ([rocks openDatabaseReadOnly:NO withTTL:ttl] == NO) {
		return nil;
	}
	return rocks;
}
			...
			andDatabaseOptions:(void (^)(RocksDBDatabaseOptions *options))optionsBlock
{
    ...
    if ([rocks openColumnFamilies:descriptor readOnly:NO withTTL:ttl] == NO) {
		return nil;
	}
	return rocks;
}
							 ...
							 andDBOptions:(void (^)(RocksDBOptions *options))optionsBlock
{
	...
	if ([rocks openDatabaseReadOnly:YES withTTL:ttl] == NO) {
		return nil;
	}
	return rocks;
}
					   ...
					   andDatabaseOptions:(void (^)(RocksDBDatabaseOptions *options))optionsBlock
{
    ...
    if ([rocks openColumnFamilies:descriptor readOnly:YES withTTL:ttl] == NO) {
		return nil;
	}
	return rocks;
}
...
- (BOOL)openDatabaseReadOnly:(BOOL)readOnly withTTL:(int32_t)ttl
{
	rocksdb::Status status;
    status = rocksdb::DBWithTTL::Open(_options.options, _path.UTF8String, &_db, ttl, readOnly);
                   ...
                   withTTL:(int32_t)ttl
{
    ...
    std::vector<int32_t> ttls(columnFamilies->size(), ttl);
    status = rocksdb::DBWithTTL::Open(_options.options,
                                      _path.UTF8String,
                                      *columnFamilies,
